resource "google_service_account" "app-cluster-sa" {
  account_id   = "app-cluster-sa"
  display_name = "App Cluster Node Service Account"
}
...
resource "google_container_node_pool" "app_cluster_linux_node_pool" {
  ...
  node_config {
    ...
    service_account = google_service_account.app-cluster-sa.email
    oauth_scopes = [
